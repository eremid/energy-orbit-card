...
const eocIcons = {
  ...
  'grid-export': 'M12,2L4.5,20.29L5.21,21L12,18L18.79,21L19.5,20.29L12,2Z',
  'battery': 'M16,20H8V6H16M16.67,4H15V2H9V4H7.33A0.67,0.67 0 0,0 6.67,4.67V21.33A0.67,0.67 0 0,0 7.33,22H16.67A0.67,0.67 0 0,0 17.33,21.33V4.67A0.67,0.67 0 0,0 16.67,4Z',
  'battery-charging': 'M11,20V13H8L13,4V11H16L11,20M16.67,4H15V2H9V4H7.33A0.67,0.67 0 0,0 6.67,4.67V21.33A0.67,0.67 0 0,0 7.33,22H16.67A0.67,0.67 0 0,0 17.33,21.33V4.67A0.67,0.67 0 0,0 16.67,4Z',
...
class EnergyOrbitCard extends HTMLElement {
  ...
  _updateContent() {
    ...
    if (batAutoEl) {
        ...
        const gridPct = gridAbs / this.config.grid_max;
        const batPowerPct = Math.abs(batteryPower) / batteryPowerMax;

        let overlap = false;
        if (isBidi) {
            // In bidi mode, negative flows (export/charge) grow counter-clockwise (left side)
            if (isInjection && gridPct > 0.2) overlap = true;
            if (isCharging && batPowerPct > 0.2) overlap = true;
        } else {
            // In normal mode, all flows grow clockwise and overlap the left side when > 80%
            if (gridPct > 0.8 || batPowerPct > 0.8) overlap = true;
        }

        // Hide subtext if battery > 70% to avoid overlap with the outer ring on desktop
        // Or if gauges overlap with text in current mode
        if (batteryPercent > 70 || overlap) {
